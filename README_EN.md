...
- **The IME candidate window can only be approximately centered**: it anchors to the caret position of the hidden staging textarea and grows rightward from there; the browser never exposes the popup's actual width to the page, so pixel-perfect centering isn't possible — only an offset based on an assumed "typical" candidate-window width.
